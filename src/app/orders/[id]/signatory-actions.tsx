...
export default function SignatoryActions({ orderId }: Props) {
  ...
  async function handleApprove() {
    if (!confirm("לאשר הזמנה זו ולשלוח לאישור מנהל?")) return;
    setLoading(true);
    setError("");
    try {
      const res = await fetch(`/api/v1/orders/${orderId}/approve-signatory`, { method: "POST" });
      if (!res.ok) {
  ...
  async function handleReject() {
    ...
    setError("");
    try {
      const res = await fetch(`/api/v1/orders/${orderId}/reject-signatory`, {
        method: "POST",
  ...
  return (
    <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-5 mb-6">
      <h3 className="font-semibold text-yellow-800 mb-1">ממתין לאישורך</h3>
      <p className="text-yellow-700 text-sm mb-4">
        בדוק את פרטי ההזמנה ואשר או דחה אותה.
      </p>
